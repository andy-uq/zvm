module Instruction
  ...
  let decode story (Instruction address) =
    ...
    * In extended form, ... the opcode number is given in a second opcode byte. *)
     ...
     * If the operand count is 0OP then the 0OP opcode number is given
       the lower 4 bits.
     * If the operand count is 1OP then the 1OP opcode number is given
       the lower 4 bits.
     * if the operand count is 2OP then the 2OP opcode number is given
       the lower 5 bits
     * If the operand count is VAR then the VAR opcode number is given
       the lower 5 bits
    *)
    ...
    let decode_branch branch_code_address opcode ver  =
      if has_branch opcode ver then
        ...
        let branch =
          ...
          | 1 -> (sense, Return_true)
          | _ ->
            let branch_length = if fetch_bit bit6 high then 1 else 2
            let (Byte_address address_after) = inc_byte_addr_by branch_code_address branch_length
